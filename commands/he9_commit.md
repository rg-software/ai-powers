...
**Project adapter.** Read `.opencode/powers.jsonc` if present for `specs`, `conventions`, `tracker`. The `specs` key names the canonical spec location to update when behavior changes.
