...
- `triage` (default, or empty): review and refine the existing backlog.
...
1. Identify the scope from the argument; if none was given, ask which domain or folder to scan.
2. Inspect it for debt, across code/architecture and documentation:
   ...
   - **Docs → Code (accuracy):** read the canonical docs for the scope (the resolved specs glob, plus relevant docs glob) and cross-check their claims against the actual source. Flag outdated claims, missing/shipped features, references to deleted systems.
   ...
   - Not all docs are spec-based: design notes under the resolved docs glob are sources too. Canonical technical truth lives under the specs glob; design intent may live elsewhere.
...
5. Update the resolved debt backlog file: add new items, refine existing ones, avoid duplicates.
