...
**Project adapter.** Read `.opencode/powers.jsonc` if present for `baseBranch`, `conventions`, `tracker`. Defaults: `baseBranch` from `origin/HEAD` else `main`; `tracker` inferred from the remote. Branch naming rules live in the adapter's `conventions` file when present.
